...
export default class Input_1 extends React.Component {
  ...
  //滚动改变视差
  //之前设置用来动态改变全屏图标位置的函数,在更改元素结构后不需要这个函数了
  scrollParallax(){
    ...
    area.addEventListener('scroll',function(event){
     ...
     // 将cancenBubble属性设置为false即可
      ...
      let scrollPos = parseFloat(area.scrollTop)

      //pc端背景缩放
      let pcSizeSetting = function(){
        if(scrollPos>0){
          valley.style.backgroundSize = 100 + scrollPos + '%'
        }else{
           valley.style.backgroundSize = "cover"
        }
      }

      //移动端背景缩放
      let phoneSizeSetting = function(){
        if(scrollPos>0){
      ...
      _this.devicePixelRatio < 1.5 ?pcSizeSetting() :phoneSizeSetting()
  ...
  togglefullScreen(e){
    ...
    if(!this.state.toggleFullScreen){
      Object.assign(this.refs['areaWrapper'].style
  ...
  render(){
    return (
          <Fragment>
            ...
            <div ref='areaWrapper' className={Css['areaWrapper']}>

              <div ref={(ref)=>{this.fullScreen = ref}} className={Css['fullScreen']} onClick={e=>this.togglefullScreen(e)}>
